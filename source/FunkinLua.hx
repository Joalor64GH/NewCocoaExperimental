...
class FunkinLua
{
	...
	public function new(script:String, ?scriptItself:String)
	{
		...
		var difficultyName:String = CoolUtil.difficultyStuff[PlayState.storyDifficulty];
		...
		set('ratingName', '');
		set('ratingFC', '');
		set('version', MainMenuState.cocoaVersion.trim());
		...
		// Gameplay settings
		@:privateAccess set('healthGainMult', PlayState.instance.gainMultiplier);
		@:privateAccess set('healthLossMult', PlayState.instance.lossMultiplier);
		@:privateAccess set('instakillOnMiss', PlayState.instance.instakillEnabled);
		...
		set('downscroll', FunkySettings.downScroll);
		set('middlescroll', FunkySettings.middleScroll);
		set('framerate', FunkySettings.framerate);
		set('ghostTapping', FunkySettings.ghostTapping);
		set('hideHud', FunkySettings.hideHud);
		set('timeBarType', FunkySettings.timeStyle);
		set('scoreZoom', FunkySettings.scoreTween);
		set('lowQuality', FunkySettings.lowGraphics);
		...
		Lua_helper.add_callback(lua, "callOnLuas",
			function(?funcName:String, ?args:Array<Dynamic>, ignoreStops = false, ignoreSelf = true, ?exclusions:Array<String>)
			{
				...
				PlayState.instance.callOnLuas(funcName, args);
			});
		...
		Lua_helper.add_callback(lua, "loadSong", function(?name:String = null, ?difficultyNum:Int = -1)
		{
			...
			PlayState.SONG = Song.loadFromJson(poop, difficultyNum);
		...
		Lua_helper.add_callback(lua, "noteTweenX", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String)
		{
			...
			if (note < 0)
				note = 0;
			var testicle:BabyArrow = PlayState.instance.strumLineNotes.members[note % PlayState.instance.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "addScore", function(value:Int = 0)
		{
			...
			PlayState.instance.calculateRating();
		});
		...
		{
			return PlayState.instance.misses;
		});
		...
		{
			var charType:Int = 0;
			PlayState.instance.addCharacterToList(name, type);
		});
		...
		{
			@:privateAccess PlayState.instance.endSong();
			return true;
		});
		...
		Lua_helper.add_callback(lua, "exitSong", function(?skipTransition:Bool = true)
		{
			if (skipTransition)
			{
		...
		{
			PlayState.instance.accuracy = value;
		});
		...
		Lua_helper.add_callback(lua, "makeBGSprite", function(tag:String, image:String, x:Float, y:Float, ?scrollX:Float = 1, ?scrollY:Float = 1)
		{
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leBGSprite:ModchartBGSprite = new ModchartBGSprite(x, y);
			if (image != null && image.length > 0)
				leBGSprite.loadGraphic(Paths.image('stages/$image'));
		...
		Lua_helper.add_callback(lua, "makeLuaGroup", function(tag:String)
		{
			...
			var leGroup:FlxTypedSpriteGroup<ModchartSprite> = new FlxTypedSpriteGroup<ModchartSprite>();
			leGroup.antialiasing = !FunkySettings.noAntialiasing;
		...
		Lua_helper.add_callback(lua, "addLuaGroup", function(tag:String, front:Bool = false)
		{
			if (PlayState.instance.modchartGroups.exists(tag))
			{
				...
				else
				{
					...
					{
						GameOverSubstate.instance.insert(GameOverSubstate.instance.members.indexOf(GameOverSubstate.instance.bf), leGroup);
					}
					else
					{
		...
		Lua_helper.add_callback(lua, "setScrollFactorGroup", function(tag:String, ?scrollX:Float = 1, ?scrollY:Float = 1)
		{
			var leGroup:FlxTypedSpriteGroup<ModchartSprite> = PlayState.instance.modchartGroups.get(tag);
		...
		Lua_helper.add_callback(lua, "playAnimGroup", function(tag:String, animation:String, force:Bool = false, reversed:Bool = false, frame:Int = 0)
		{
			var leGroup:FlxTypedSpriteGroup<ModchartSprite> = PlayState.instance.modchartGroups.get(tag);

			if (leGroup != null)
			{
				leGroup.forEachAlive(function(sprite:ModchartSprite)
				{
					sprite.playAnim(animation, force, reversed, frame);
				});
			}
		});
		...
		Lua_helper.add_callback(lua, "setGroupSize", function(tag:String, x:Int, y:Int = 0, ?updateHitbox:Bool = true)
		{
			var leGroup:FlxTypedSpriteGroup<ModchartSprite> = PlayState.instance.modchartGroups.get(tag);
		...
		Lua_helper.add_callback(lua, "updateHitboxGroup", function(tag:String)
		{
			var leGroup:FlxTypedSpriteGroup<ModchartSprite> = PlayState.instance.modchartGroups.get(tag);
		...
		Lua_helper.add_callback(lua, "makeGraphicRGB", function(obj:String, width:Int, height:Int, color:Array<Int>)
		{
			var color:FlxColor = FlxColor.fromRGB(color[0], color[1], color[2]);
			...
			if (PlayState.instance.modchartSprites.exists(tag))
			{
				...
				if (!shit.wasAdded)
				{
					...
					else
					{
						if (PlayState.instance.isDead)
						{
							GameOverSubstate.instance.insert(GameOverSubstate.instance.members.indexOf(GameOverSubstate.instance.bf), shit);
						}
						else
						{
							var position:Int = PlayState.instance.members.indexOf(PlayState.instance.gfGroup);
		...
		Lua_helper.add_callback(lua, "checkFileExists", function(filename:String, ?absolute:Bool = false)
		{
			...
			var path:String = Paths.mods(filename);
			if (FileSystem.exists(path))
			{
				return true;
			}
		...
		Lua_helper.add_callback(lua, "deleteFile", function(path:String, ?ignoremods:Bool = false)
		{
			try
			{
				if (!ignoremods)
				{
					var lePath:String = Paths.mods(path);
		...
		Lua_helper.add_callback(lua, "getTextFromFile", function(path:String, ?ignoremods:Bool = false)
		{
			return Paths.getTextFromFile(path, ignoremods);
		});
...
class ModchartBGSprite extends ModchartSprite
{
