...
class OldChartingState extends MusicBeatState
{
	...
	**/
	...
	function addEventsUI():Void
	{
		...
		descText = new FlxText(20, 245, 0, eventStuff[0][0]);
	...
	override function update(elapsed:Float)
	{
		...
		curRenderedEvents.forEachAlive(function(event:EventNote)
		{
			...
			if (isSameEvent(event, curSelectedEvent))
			{
				...
				var colorVal:Float = 0.7 + Math.sin((Math.PI * eventSine) / 180) * 0.3;
		...
		curRenderedNotes.forEachAlive(function(note:Note)
		{
			...
			if (note.strumTime <= Conductor.songPosition)
			{
					...
					&& note.noteData > -1)
				{
					...
					strumGroup.members[noteDataToCheck].holdTimer = note.sustainLength / 1000 + 0.15;
