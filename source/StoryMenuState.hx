...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bgSprite.antialiasing = !FunkySettings.noAntialiasing;
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			weekThing.antialiasing = !FunkySettings.noAntialiasing;
			...
			if (weekIsLocked(i))
			{
				...
				lock.ID = i;
				lock.antialiasing = !FunkySettings.noAntialiasing;
				grpLocks.add(lock);
			}
		}
		...
		leftArrow.antialiasing = !FunkySettings.noAntialiasing;
		...
		for (i in 0...CoolUtil.difficultyStuff.length)
		{
			var sprDifficulty:FlxSprite = new FlxSprite(leftArrow.x + 60,
				leftArrow.y).loadGraphic(Paths.image('menudifficulties/' + CoolUtil.difficultyStuff[i].toLowerCase()));
			sprDifficulty.x += (308 - sprDifficulty.width) / 2;
			sprDifficulty.ID = i;
			sprDifficulty.antialiasing = !FunkySettings.noAntialiasing;
		...
		rightArrow.antialiasing = !FunkySettings.noAntialiasing;
		...
		tracksSprite.antialiasing = !FunkySettings.noAntialiasing;
		add(tracksSprite);
	...
	function selectWeek()
	{
		if (!weekIsLocked(curWeek))
		{
			...
			if (map != null)
			{
				try 
				{	
					var data:Dynamic = map.get(WeekData.weeksList[curWeek]);
					var song:String = data.song;
					var index:Int = data.index;
					var copy:Array<String> = songArray.copy();
					var oldCopy:Array<String> = copy.copy();
					for (i in copy)
					{
						var intIndex:Int = copy.indexOf(i);
						copy.remove(i);
						i = CoolUtil.coolSongFormatter(i);
						copy.insert(intIndex, i);
					}
					var otherSong:String = copy[index];
					for (i in 0...index)
					{
						if (index > 0)
						{
							var thing:String = oldCopy[i];
							oldCopy.remove(thing);
						}
						else
							break;
					}
					if (song == otherSong && copy[0] != song)
					{
						persistentUpdate = false;
						openSubState(new ContinueSubstate(song, curDifficulty, oldCopy.copy(), curWeek, songArray.copy()));
						return;
					}
				}
			...
			{
				// weekSave.data.week = new Map<String, Dynamic>();
				// weekSave.flush();
			}
